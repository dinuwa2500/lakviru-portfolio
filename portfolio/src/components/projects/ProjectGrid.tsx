...
export function ProjectGrid({
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full h-10 pl-10 pr-9 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors"
          />
          {searchQuery && (
            <button
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            <select
              ...
              className="h-10 px-3 pr-8 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 text-xs font-medium text-zinc-700 dark:text-zinc-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 cursor-pointer"
            >
          ...
          {/* Sort dropdown */}
          <div className="flex items-center gap-1.5 bg-white/80 dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 rounded-lg px-2.5 h-10">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="h-10 px-3 rounded-lg border border-rose-500/20 text-rose-600 dark:text-rose-400 bg-rose-500/10 hover:bg-rose-500/20 text-xs font-medium transition-colors flex items-center gap-1 cursor-pointer"
            >
      ...
      {/* Category Tabs */}
      <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-zinc-200/80 dark:border-zinc-800/80 scrollbar-none">
        {categories.map((cat) => (
          <button
            key={cat}
            onClick={() => setSelectedCategory(cat)}
            className={cn(
              'px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap transition-all cursor-pointer',
